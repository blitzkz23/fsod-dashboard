...
global names
names = load_classes('data/weapon.names')
...
def load_yolor_and_process_each_frame_video(vid_name, enable_GPU, confidence, assigned_class_id, kpi1_text, kpi2_text,
                                      kpi3_text, stframe):
    data_deque.clear()
    ...
    out, source, weights, save_txt, imgsz, cfg = \
        'inference/output', vid_name, 'best_overall.pt', False, 1280, 'cfg/yolor_custom.cfg'
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        kpi2_text.write(f"<h1 style='text-align:center;color:red;'> {len(data_deque)}</h1>",
                        unsafe_allow_html=True)
        kpi3_text.write(f"<h1 style='text-align:center;color:red;'> {w}</h1>",
                        unsafe_allow_html=True)
...
def load_yolor_and_process_each_frame_webcam(vid_name, enable_GPU, confidence, assigned_class_id, kpi1_text, kpi2_text,
                                      kpi3_text, stframe):
    data_deque.clear()
    speed_four_line_queue.clear()
    object_counter.clear()
    ...
    out, source, weights, save_txt, imgsz, cfg = \
        'inference/output', vid_name, 'best_overall.pt', False, 640, 'cfg/yolor_custom.cfg'
    webcam = source == '0' or source.startswith('rtsp') or source.startswith('http') or source.endswith('.txt')
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        kpi2_text.write(f"<h1 style='text-align:center;color:red;'> {len(data_deque)}</h1>",
                        unsafe_allow_html=True)
        kpi3_text.write(f"<h1 style='text-align:center;color:red;'> {640}</h1>",
                        unsafe_allow_html=True)
        stframe.image(im0, channels='BGR', use_column_width=True)
...
def detect_img(file_name, enable_GPU, confidence, assigned_class_id,
           stframe, kpi1_text):
    data_deque.clear()
    speed_four_line_queue.clear()
    object_counter.clear()
    ...
    prevTime = 0
    out, source, weights, save_txt, imgsz, cfg = \
        'inference/output', file_name, 'best_overall.pt', False, 1280, 'cfg/yolor_custom.cfg'
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # data_deque assign inside yolor.py
        kpi1_text.write(f"<h1 style='text-align:center;color:red;'> {len(det)}</h1>",
                        unsafe_allow_html=True)
